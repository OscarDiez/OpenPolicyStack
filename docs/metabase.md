...
The metabase webserver is running on port 3000 and 3000 is forwarded to the Commission network (ask Sandrino, he set this up). This means the metabase dashboard can be accessed from any corporate laptop in the Commission. The URL is:

```http://quantum-monitor.cnect-srv4dev.s4dad.aws.cloud.tech.ec.europa.eu:3000/```



## Admin Login

Access the dashboard [here](http://quantum-monitor.cnect-srv4dev.s4dad.aws.cloud.tech.ec.europa.eu:3000/) from your corporate device and use

```
Username: cnect-c2@ec.europa.eu
Password: Quantum2025
```
as credentials. Note that the server is running between 7:30am and 10:30pm from Monday to Friday. 
